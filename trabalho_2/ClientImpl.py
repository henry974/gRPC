...
def receive_messages(stub, sub_key, name):
    ...
    try:
        # Loop para processar o streaming de respostas (equivalente a receive(Cell o))
...
def main():
    ...
    if A != 0:
        # Roda o subscriber em thread paralela para não bloquear a publicação[cite: 3]
        threading.Thread(target=receive_messages, args=(stub, A, name), daemon=True).start()

    if B != 0:
        # Cria uma nova Cell com o valor de A, conforme a lógica Java
